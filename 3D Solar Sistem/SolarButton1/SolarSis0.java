...
{
	private PickCanvas pickCanvas;//para seleccionar objetos de el universo
	private JComboBox jcb;//para seleccionarlos mediante un combobox
...
frame.addWindowListener(new WindowAdapter() {
    ...
    JPanel jp=new JPanel();//usamos un panel
    frame.setLayout(new BorderLayout());//agregamos un layout al frame
    Vector nomPlanet=new Vector();
    nomPlanet.addElement("Sol");
    nomPlanet.addElement("Tierra");
    nomPlanet.addElement("Marte");
    jcb=new JComboBox(nomPlanet);//añadimos los nombres al combo box
    jcb.addActionListener(this);//
    jta=new JTextArea("informacion",5,20);
    
    jp.add(jcb);//agregamos al panel el combobox
    jp.add(jta);
    frame.add("Center",canvas);//ubicamos el canvas al frame
    frame.add("South", jp);//ubicamos el panel 
